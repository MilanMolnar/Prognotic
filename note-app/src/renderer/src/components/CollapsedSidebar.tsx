...
import { useBlockActions, useCalendar, useCalendarActions, useGoalActions, useGoals, useI18n, usePanelActions } from '@renderer/context'
import { cn } from '@renderer/utils'
import { countPendingCalendarItems, hasUncertainCalendarItems } from '@shared/calendar'
import { JSX } from 'react'
import { LuBookMarked, LuCalendarDays, LuPanelLeftOpen } from 'react-icons/lu'
...
export const CollapsedSidebar = (): JSX.Element => {
  ...
  const { isCalendarSelected, isGlossarySelected } = useGoals()
  const { selectCalendar, selectGlossary } = useGoalActions()
    ...
    <div className="flex h-full flex-col items-center justify-between pb-1">
      <div className="flex flex-col items-center gap-2 pt-1">
        ...
        <ActionButton
          ...
          className={cn(isGlossarySelected && 'border-yellow-500/60')}
        >
          <LuBookMarked className={cn('h-4 w-4', isGlossarySelected ? 'text-yellow-500' : 'text-zinc-300')} />
        </ActionButton>
      </div>
      <div className="flex flex-col gap-2">
        <SettingsButton />
        <ActionButton onClick={toggleLeftPanel} title={t('navigation.expandSidebar')}>
